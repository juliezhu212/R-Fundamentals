...
Before we get started, let's load in the `tidyverse` package that we used in Part 2. We will use this today to subset our data and to make plots!
...
```{r}
new_subset <- select(gap, country, gdpPercap, pop)
...
gap %>%
  # filter to Mexico
  # note, we don't need to specify gap inside select() any more
...
```{r}
# save Mexico rows (from above exercise) to another data frame
data_mexico <- gap %>%
  # filter to Mexico
  filter(country == "Mexico") %>%
...
# the code below subtracts the number of rows in gap_nonmiss from number of rows in gap
# this is how many rows filter() dropped
...
# otherwise, need to call table() on it
